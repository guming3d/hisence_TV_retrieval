...
| 源 | 角色 | 交付方式 | 频率 | 覆盖字段 |
|---|---|---|---|---|
| **IMDB** | 权威主数据 (master) | 手动批量 JSON/CSV 落地 ADLS `/raw/imdb/{batch_id}.json`,blob trigger | 运营驱动,典型月度/季度 | 规范 title、描述、imdb_rating、cast、genres、release_year、kind |
...
titles            (id, imdb_id?, primary_title, normalized_title,
                   kind, release_year, series_id?,
                   vod_playable, rag_doc_version,
                   content_hash, merge_version,
                   genres_merged, cast_merged,
                   updated_at)
...
**关键索引** (增量):
- `UNIQUE(imdb_id) WHERE imdb_id IS NOT NULL ON titles` → 去重 Stage 1
...
| `genres_merged` | **W (主类别)** | append 补充 | — | 集合 union,多类别允许并存 |
| `release_year` | **W** | R | — | IMDB 权威 |
| `cast_merged` | **W** | — | — | IMDB only |
...
| 去重策略 | imdb_id 精确 → 归一化 title pg_trgm 模糊 (阈值 0.85 + kind + year±1) | 兼顾召回与假阳性控制 |
...
| 4 | 去重阈值 (Stage 2) | pg_trgm similarity ≥ 0.85 + 同 kind + year ±1 | 无阈值 (假阳泛滥) / 纯 embedding 相似度 (成本高) |
...
| 8 | 默认 LLM | gpt-5-mini | gpt-4o-mini (更便宜) / gpt-4o (更准) |
